...
### 技术分析（6 维度量化评分 + 多指标详细分析）

| 维度 | 权重 | 核心指标 |
|------|------|----------|
| 趋势强度 | 30% | ADX、均线排列（MA5/MA20/MA50/MA200）、MA200 位置 |
| 动量指标 | 20% | RSI（含背离检测）、MACD（金叉/死叉/零轴/背离/柱状图趋势）、KDJ（金叉/死叉/钝化）、ROC |
| 波动率状态 | 15% | ATR 比率、布林带宽度 & %B & 挤压检测 & 轨道行走 |
| 量价配合 | 15% | OBV、涨跌量比、成交量活跃度 |
| 支撑阻力 | 10% | 布林带位置、均线支撑、枢轴点 |
| 相对强度 | 10% | 20/60 日价格变动率 |

每个指标的详细分析（含金叉/死叉、顶底背离、超买超卖、挤压/变盘信号）均以结构化表格形式呈现在报告中。
...
9. **技术分析**
   - 六维度评分表（含权重与加权得分）
   - 各指标详细中文描述（MA / RSI / MACD / KDJ / 布林带）
   - KDJ 指标分析表（K/D/J 值、金叉死叉、高位/低位钝化、超买超卖）
   - MACD 指标分析表（快慢线、零轴位置、柱状图趋势、顶底背离检测）
   - RSI 指标分析表（区间定位、趋势方向、50 中轴穿越、顶底背离）
   - 布林带指标分析表（%B、带宽、挤压检测、轨道行走、反转信号、中轨倾斜）
   - 关键支撑阻力位（ASCII 价位图）
   - 交易信号汇总（6 指标：MA/RSI/布林带/MACD/KDJ/综合）
...
### 2026-04-27 技术分析深度增强：KDJ + MACD 背离 + RSI 背离 + 布林带挤压

1. **新增 KDJ 指标完整分析**
   - 计算 K/D/J 三值（J=3K-2D），独立于原有随机指标
   - 金叉/死叉检测：K 线上穿 D 线为金叉，下穿为死叉
   - 超买超卖判断：J 值 >100 超买，<0 超卖；K 值 >80/<20 高位/低位
   - 高位/低位钝化识别（K>80&D>80&J>100 或 K<20&D<20&J<0）
   - 图表新增第 4 面板：KDJ 三线走势图（含超买超卖参考线）

2. **MACD 分析深度增强**
   - 金叉/死叉状态 + 零轴位置（零轴上/下方）
   - 柱状图四态分类：多头放量/多头缩量/空头放量/空头缩量
   - 柱状图连续趋势：连续增长/连续衰减/震荡变化
   - **顶底背离检测**：价格创新高但 MACD 柱未创新高 → 顶背离预警；反之底背离

3. **新增 RSI 结构化分析**
   - 四区间定位：超买(≥70) / 偏强(50-70) / 偏弱(30-50) / 超卖(≤30)
   - 趋势方向：近 5 期连续上升/下降检测
   - **50 中轴穿越信号**：上穿 50 由弱转强，下破 50 由强转弱
   - **RSI 背离检测**：经典顶底背离 + 隐藏顶底背离（需 30 期数据）

4. **新增布林带结构化分析**
   - **%B 指标**：价格在轨道内的归一化位置
   - **带宽与挤压检测**：与 20 期最小带宽比较（极度收窄<1.10x / 收窄中<1.25x / 扩张>1.5x）
   - **轨道行走检测**：近 5 期触及上轨/下轨 ≥4 次为行走信号
   - **反转信号**：触下轨后回升至中轨上 = 多头反转；触上轨后回落至中轨下 = 空头反转
   - **中轨倾斜方向**：近 5 期斜率判断

5. **报告章节扩展**
   - 9.3 KDJ 指标分析 | 9.4 MACD 指标分析 | 9.5 RSI 指标分析 | 9.6 布林带指标分析
   - 每项均以结构化表格展示，含数值与中文判断
   - 9.2 技术指标详情含全部 5 指标的逗号连接中文描述
   - 9.7 关键支撑阻力位（ASCII 价位图）
   - 9.8 交易信号汇总（6 信号：MA/RSI/布林带/MACD/KDJ/综合）

6. **风控警告增强**（`agent_bridge.py`）
   - KDJ 金叉/死叉、超买超卖、高位/低位钝化
   - MACD 顶底背离预警、RSI 顶底背离、RSI 中轴穿越
   - 布林极度收窄变盘信号、轨道行走、反转信号

7. **AI 分析透传**（`ai_analyzer.py`）
   - DeepSeek prompt 新增 MACD 背离、KDJ 金叉死叉、RSI 背离、布林挤压等字段
